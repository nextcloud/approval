OC.L10N.register(
    "approval",
    {
    "You approved {file}" : "Je accordeerde {file}",
    "{user} approved {file}" : "{user} accordeerde {file}",
    "You rejected {file}" : "Je weigerde {file}",
    "{user} rejected {file}" : "{user} weigerde {file}",
    "Your approval was requested on {file}" : "Je toestemming is gevraagd voor {file}",
    "Your approval was requested on {file} by {who}" : "Je toestemming voor {file} is gevraagd door {who}",
    "You requested approval on {file}" : "Je hebt verzocht goedkeuring voor {file}",
    "A guest user" : "Een gast",
    "Approval" : "Goedkeuring",
    "<strong>Approval</strong> events" : "<strong>Goedkeurings</strong>activiteiten",
    "Error getting OAuth access token" : "Fout bij het verkrijgen van een OAuth toegangstoken",
    "Error during OAuth exchanges" : "Fout tijdens OAuth uitwisselingen",
    "A file was approved" : "Een bestand is goedgekeurd",
    "A file was rejected" : "Een bestand is geweigerd",
    "A directory was rejected" : "Een map is geweigerd",
    "%1$s approved %2$s" : "%1$s keurde %2$sgoed",
    "%1$s rejected %2$s" : "%1$s keurde %2$s af",
    "{user} approved {node}" : "{user} keurde {node} goed",
    "{user} rejected {node}" : "{user} weigerde {node}",
    "Your approval was requested" : "Je goedkeuring is gevraagd",
    "Your approval was requested for file %1$s" : "Jouw goedkeuring voor %1$s werd gevraagd",
    "Your approval was requested for directory %1$s" : "Je goedkeuring is gevraagd voor map %1$s",
    "%2$s requested your approval for %1$s" : "%2$s vroeg je toestemming voor %1$s",
    "{user} requested your approval for {node}" : "{user} vroeg jouw goedkeuring voor {node}",
    "Please check my approval request" : "Bekijk mijn goedkeuringsaanvraag",
    "Signature of %s" : "Handtekening van %s",
    "Bad credentials" : "Foute inloggegevens",
    "OAuth access token refused" : "OAuth access token geweigerd",
    "Approval workflows" : "Goedkeuringswerkstromen",
    "Let users approve or reject files" : "Laat gebruikers bestanden goed of afkeuren",
    "Approve/reject files based on workflows defined by admins." : "Keur bestanden goed of af op basis van werkstromen gedefinieerd door administrators.",
    "Each workflow defines who (which users, groups or circles) can approve files for a given pending tag and which approved/rejected tag should then be assigned." : "Elke werkstroom definieert wie (welke gebruikers, groepen of kringen) bestanden kunnen goedkeuren voor een bepaalde tag die in behandeling is en welke goedgekeurde/afgewezen tag dan moet worden toegewezen.",
    "A list of users/groups/circles who can manually request approval can be optionally defined." : "Een lijst met gebruikers/groepen/kringen die handmatig goedkeuring kunnen aanvragen, kan optioneel worden gedefinieerd.",
    "To be considered approved, a file/directory having multiple pending tags assigned must be approved by all the workflows involved." : "Om als goedgekeurd te worden beschouwd, moet een bestand/map waaraan meerdere lopende tags zijn toegewezen, worden goedgekeurd in alle betrokken werkstromen.",
    "You can chain approval workflows by using a pending tag as approved/rejected tag in another workflow." : "Je kunt goedkeuringswerkstromen koppelen door een tag in behandeling te gebruiken als goedgekeurde/afgekeurde tag in een andere werkstroom.",
    "All tags must be different in a workflow. A pending tag can only be used in one workflow." : "Alle tags in een werkstroom moeten verschillend zijn. Een  behandelingstag kan maar door één werkstroom gebruikt worden. ",
    "No workflow yet" : "Nog geen werkstromen",
    "New workflow" : "Nieuwe werkstroom",
    "Create new hidden tag" : "Maak een verborgen tag",
    "New tag name" : "Nieuwe naam tag",
    "Create" : "Creëren",
    "Cancel" : "Annuleren",
    "All fields are required" : "Alle velden zijn verplicht",
    "All tags must be different" : "Alle tags moeten anders zijn",
    "Pending tag is already used in another workflow" : "De behandelingstag is al in gebruik door een andere werkstroom",
    "Create workflow" : "Creëer werkstroom",
    "Failed to get approval workflows" : "Kon goedkeuringswerkstromen niet ophalen",
    "Approval workflow saved" : "Goedkeuringswerkstroom opgeslagen",
    "Failed to save approval workflow" : "Kon goedkeuringswerkstroom niet bewaren",
    "New approval workflow created" : "Nieuwe goedkeuringswerkstroom aangemaakt",
    "Failed to create approval workflow" : "Kon goedkeuringswerkstroom niet creëren",
    "Approval workflow deleted" : "Goedkeuringswerkstroom verwijderd",
    "Failed to delete approval workflow" : "Kon goedkeuringswerkstroom niet verwijderen",
    "Tag \"{name}\" created" : "Tag \"{name}\" gemaakt",
    "Failed to create tag \"{name}\"" : "Aanmaken tag \"(name)\" mislukt",
    "Approve" : "Goedkeuren",
    "Reject" : "Afkeuren",
    "Select pending tag" : "Select onderhanden tag",
    "Who can request approval?" : "Wie kan toestemming vragen?",
    "Who can approve?" : "Wie kan goedkeuren?",
    "Select approved tag" : "Selecteer goedkeuringstag",
    "Select rejected tag" : "Selecteer verwijderde tag",
    "Delete workflow" : "Verwijder werkstroom",
    "Who can request approval" : "Wie kan toestemming vragen",
    "Who can approve" : "Wie kan goedkeuren",
    "Tag to act on" : "Tag om aan te passen",
    "Tag set on approval" : "Tag ingesteld na goedkeuring",
    "Tag set on rejection" : "Tag ingesteld na afwijzing",
    "Workflow title" : "Werkstroomtitel",
    "What is the purpose of this workflow?" : "Wat is het doel van deze werkstroom?",
    "Approved" : "Goedgekeurd",
    "Approved by" : "Goedgekeurd door",
    "Rejected" : "Afgekeurd",
    "Rejected by" : "Afgewezen door",
    "Approval requested by" : "Goedkeuring gevraagd door",
    "Pending approval" : "Wacht op goedkeuring",
    "you" : "jij",
    "Request approval" : "Vraag toestemming",
    "Sign with DocuSign" : "Ondertekenen met DocuSign",
    "Sign with LibreSign" : "Ondertekenen met LibreSign",
    "Your approval was requested by {name}." : "{name} vroeg jouw goedkeuring",
    "The related approval workflow is:" : "De gerelateerde goedkeuringswerkstroom is:",
    "Your approval was requested." : "Jouw goedkeuring werd gevraagd",
    "Request a signature via DocuSign" : "Vraag ondertekening via DocuSign",
    "Users or email addresses" : "Gebruikers of e-mailadressen",
    "Nextcloud users or email addresses" : "Nextcloud gebruikers of e-mailadressen",
    "Recipients will receive an email from DocuSign with a link to sign the document. You will be informed by email when the document has been signed by all recipients." : "Ontvangers ontvangen een e-mail van DocuSign met een link om het document te ondertekenen. Je wordt per e-mail geïnformeerd wanneer het document door alle ontvangers is ondertekend.",
    "Request signature" : "Aanvragen ondertekening",
    "Recipients will receive an email from DocuSign to sign the document" : "Ontvangers zullen een email van DocuSign ontvangen om het document te ondertekenen",
    "Failed to request signature with DocuSign" : "Aanvraag voor ondertekening via DocuSign mislukt",
    "DocuSign integration" : "DocuSign integratie",
    "DocuSign is an electronic signature solution." : "DocuSign is een elektronische ondertekeningstool.",
    "If you want to use DocuSign, create an application in your DocuSign \"My Apps & Keys\" developer account settings and put the client ID (integration key) and secret below." : "Als je DocuSign wilt gebruiken, maak een applicatie in je DocuSign \"My Apps & Keys\" ontwikkelingsopties en plaats de client ID (integration key) en secret hieronder.",
    "Make sure you set this \"Redirect URI\":" : "Zorg ervoor dat je deze \"Redirect URI\" ingestelt",
    "If your DocuSign user is associated with multiple DocuSign accounts, the default one will be used." : "Als je DocuSign-gebruiker is gekoppeld aan meerdere DocuSign-accounts, dan wordt het standaardaccount gebruikt.",
    "Client ID of your application" : "Client ID van je applicatie",
    "Application secret key" : "Application secret key",
    "Secret key of your application" : "Secret key van je applicatie",
    "Connect to DocuSign" : "Verbinden met DocuSign",
    "Connected as {user} ({email})" : "Verbonden als {user} ({email})",
    "Disconnect from DocuSign" : "Ontkoppelen van DocuSign",
    "Successfully connected to DocuSign!" : "Succesvol verbonden met DocuSign!",
    "OAuth access token could not be obtained:" : "OAuth access token kon niet worden opgehaald:",
    "DocuSign admin options saved" : "Docusign beheeropties bewaard",
    "Failed to save DocuSign admin options" : "Kon DocuSign admin-opties niet opslaan",
    "Failed to save DocuSign OAuth state" : "Kan DocuSign OAuth-status niet bewaren",
    "No recommendations. Start typing." : "Geen aanbevelingen. Begin te typen.",
    "No result." : "Geen resultaat.",
    "Who?" : "Wie?",
    "Select approval workflow" : "Selecteer goedkeuringswerkstroom",
    "File will be automatically shared with everybody allowed to approve." : "Bestand wordt automatisch gedeeld met iedereen die mag goedkeuren.",
    "Can be approved by" : "Kan worden goedgekeurd door",
    "approved {name}" : "goedgekeurd {name}",
    "Failed to approve {name}" : "Kon {name} niet goedkeuren",
    "Rejected {name}" : "{name} afgekeurd",
    "Warning" : "Waarschuwing",
    "Failed to request approval for {name}" : "Kon geen goedkeuren vragen voor {name} ",
    "The user who requested this approval was not found, remember to send or share the signed document yourself" : "De gebruiker die deze goedkeuring heeft aangevraagd, is niet gevonden, vergeet niet het ondertekende document zelf te verzenden of te delen",
    "Approval signature" : "Goedkeuringshandtekening",
    "{name} signature requested via LibreSign!" : "{name} ondertekening aangevraagd via LibreSign!",
    "Failed to check approval status" : "Kon goedkeuringsstatus niet ophalen",
    "Pending approval, you are authorized to approve." : "In afwachting van goedkeuring mag jij goedkeuren.",
    "Show approval details" : "Toon goedkeuringsdetails",
    "{name} rejected!" : "{name} wees af!"
},
"nplurals=2; plural=(n != 1);");
